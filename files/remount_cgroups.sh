...
# If /remount_cgroups.done exists then we have already done our job
# we cannot return or "exit" since we were sources, so wrap in an IF
#
if [ ! -f /tmp/remount_cgroups.done ]; then
